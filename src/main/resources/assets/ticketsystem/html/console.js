...
(function(){
  ...
  function renderFareTable(type, fares, stations, lines) {
    ...
    table.setAttribute('data-fare-type', type);
    ...
    cornerTh.innerHTML = '<div class="label-cn">车站</div><div class="label-en">Station</div>';
    ...
    for (const ga of displayGroups) {
      ...
      rowTh.innerHTML = `<div class="label-cn">${ga.name || ga.codes[0]}</div><div class="label-en">${ga.en_name || ''}</div><div class="label-code">${(ga.codes || []).join(' | ')}</div>`;
      ...
      for (const gb of displayGroups) {
        ...
        if (best != null) {
          ...
          td.innerHTML = '<span style="color: var(--muted); font-size: 12px;">-</span>';
        }
  ...
  document.getElementById('exportChartPng')?.addEventListener('click', async () => {
    try {
      ...
      if (typeof html2canvas === 'function') {
        ...
        // 为导出优化添加特殊类
        table.classList.add('export-optimized');

        canvas = await html2canvas(table, {
          backgroundColor: bg,
          useCORS: true,
          allowTaint: true,
          scale: 4, // 提高分辨率，确保文字清晰
          width: fullW,
          height: fullH,
          windowWidth: fullW,
          windowHeight: fullH,
          ...
          onclone: (doc) => {
            ...
            if (clonedTable) {
              clonedTable.classList.add('export-optimized');
        ...
        ctx.font = `${tableStyle.fontSize || '14px'} ${tableStyle.fontFamily || 'Segoe UI, sans-serif'}`;
        ...
        rows.forEach((r, ri) => {
          ...
          cells.forEach((cell, ci) => {
            ...
            // 单元格背景色 - 直接从计算样式获取
            let cellBg = cellStyle.backgroundColor;
            // 如果背景是透明或无效，使用后备方案
            if (!cellBg || cellBg === 'rgba(0, 0, 0, 0)' || cellBg === 'transparent') {
              // 尝试获取CSS变量 --card-bg 或 --surface
              cellBg = getComputedStyle(document.body).getPropertyValue('--card-bg') ||
                      getComputedStyle(document.body).getPropertyValue('--card') ||
                      getComputedStyle(document.body).getPropertyValue('--surface') ||
                      (ri === 0 || cell.tagName === 'TH' ? '#f8f9fa' : '#ffffff');
            }
            ctx.fillStyle = cellBg;
            ctx.fillRect(x, y, w, h);

            // 边框 - 从计算样式获取边框颜色
            let borderColor = cellStyle.borderColor || cellStyle.borderTopColor;
            if (!borderColor || borderColor === 'rgba(0, 0, 0, 0)') {
              borderColor = getComputedStyle(document.body).getPropertyValue('--border') || '#e5e7eb';
            }
            ...
            // 文本颜色
            const textColor = cellStyle.color || getComputedStyle(document.body).getPropertyValue('--text') || '#1f2937';
            ...
            if (dots && dots.length) {
              ...
              const dy = y + 8;
              dots.forEach(d => {
                const dotColor = getComputedStyle(d).backgroundColor;
                if (dotColor && dotColor !== 'rgba(0, 0, 0, 0)') {
                  ctx.beginPath();
  ...
  document.getElementById('exportChartExcel')?.addEventListener('click', async () => {
    try {
      ...
      if (typeof XLSX !== 'undefined' && XLSX && XLSX.utils && XLSX.writeFile) {
        // 使用SheetJS库导出为xlsx
        const wb = XLSX.utils.table_to_book(table, { sheet: 'FareChart' });
