...
public class TicketSystemConfig {
    ...
    public static final ForgeConfigSpec.ConfigValue<String> WEB_API_BASE_URL;
    ...
    static {
        ...
        WEB_SERVER_ENABLED = BUILDER.comment("是否启用Web服务器 (默认: true)")
            .define("web_server_enabled", true);

        WEB_API_BASE_URL = BUILDER.comment("Web API基础地址 (默认: 空，使用127.0.0.1)")
            .define("web_api_base_url", "");
    ...
    public static boolean isWebServerEnabled() {
        return WEB_SERVER_ENABLED.get();
    }

    public static String getWebApiBaseUrl() {
        return WEB_API_BASE_URL.get();
    }
}