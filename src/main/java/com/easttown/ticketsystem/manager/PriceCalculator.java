...
public class PriceCalculator {
    ...
    private static int calculatePriceByFareTable(String startCode, String destCode) {
        ...
        TicketSystemMod.LOGGER.debug("No fare definition found for {} -> {}, falling back to distance calculation",
                startCode, destCode);
        int distancePrice = calculatePriceByDistance(startCode, destCode);

        // 将计算出的票价保存到票价表中，以便下次直接使用
        if (distancePrice > 0) {
            try {
                Fare newFare = new Fare(startCode, destCode, distancePrice);
                NetworkManager.addFare(newFare);
                TicketSystemMod.LOGGER.debug("Auto-generated fare for {} -> {}: {}铜币", startCode, destCode, distancePrice);
            } catch (Exception e) {
                TicketSystemMod.LOGGER.error("Failed to save auto-generated fare", e);
            }
        }
